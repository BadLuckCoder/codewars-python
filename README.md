# 🐍 Python Codewars Solutions

![Codewars Badge](https://www.codewars.com/users/BadLuckCoder/badges/micro)

[My CodeWars Profile](https://www.codewars.com/users/BadLuckCoder)

مستودع يضم حلولي لمسائل برمجية على منصة **Codewars** باستخدام لغة **Python**.

---

## 📊 التقدم (Progress)

| Kyu Level | Solved Katas |
| :---: | :---: |
| 8 KYU | 1 |
| 7 kyu | 1 |
| 6 kyu | 5 |
| 5 kyu | 1 |

---

## 🛠️ أبرز المسائل والحلول

| Kata Title | Level | Solution Link | Topic / Logic |
| :--- | :---: | :---: | :--- |
| **Maximum Subarray Sum** | `5 kyu` | [Code](./5kyu/maximum_subarray_sum.py) | Dynamic Programming, Kadane's Algorithm |
| **The Supermarket Queue** | `6 kyu` | [Code](./6kyu/supermarket_queue.py) | Simulation, Queues, Min/Max |
| **Consecutive Strings** | `6 kyu` | [Code](./6kyu/consecutive_strings.py) | Array Slicing, Strings, Sliding Window |
| **Tribonacci Sequence** | `6kyu` | [Code](./6kyu/tribonacci%20_sequence.py) | Number Theory, Arrays, Fundamentals |
| **Sort The Odd** | `6kyu` | [Code](./6kyu/sort_the_odd.py) | Fundamentals, Arrays, Sorting |
| **Your order, please** | `6kyu` | [Code](./6kyu/your_order_please.py) | String, Fundamentals |